...
// Solution inspired by theburningmunkcom
let dir = "C:/Users/olath/OneDrive/Dokument/AdventOfCodeFSharp/AdventOfCodeFSharp"
let input = File.ReadAllLines (dir + "/Day10Input.txt")
  ...
  seq {
    while Seq.length botsWithTwo > 0 do
      for bot in (Array.ofSeq botsWithTwo) do
        let lowVal = bot.Microchips |> List.sort |> Seq.last
        let hiVal = bot.Microchips |> List.sort |> Seq.head
        bots.[bot.Number] <- { bot with Microchips = [] }

        giveChipTo lowVal bot.LowTarget
        giveChipTo hiVal bot.HiTarget
